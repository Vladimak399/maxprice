...
export async function sendMessageToUser(userId: string, text: string): Promise<void> {
  for (const chunk of chunkText(text, 3900)) {
    await requestMax(`/messages?user_id=${encodeURIComponent(userId)}`, {
      method: "POST",
      body: JSON.stringify({ text: chunk, notify: true })
    });
  }
}
...
export async function sendMessage(target: { userId?: string; chatId?: string }, text: string): Promise<void> {
  if (target.chatId) {
    await sendMessageToChat(target.chatId, text);
    return;
  }

  if (target.userId) {
    await sendMessageToUser(target.userId, text);
    return;
  }
