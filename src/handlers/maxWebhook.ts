...
function isMessageCreated(updateType: string | null): boolean { return Boolean(updateType && (updateType === "message_created" || updateType.endsWith("message_created"))); }
...
async function processUpdate(update: MaxUpdate): Promise<void> {
  ...
  if (config) {
    ...
    if (config.mode === "price_changes" && extracted.chatId) { await processPriceUpdate(update, extracted.chatId); return; }
