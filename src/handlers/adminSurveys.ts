...
import { listSurveyCampaigns, sendSurveyInvite } from "../hrSurvey/broadcast";
...
async function duplicateSurvey(sourceId: string) {
  ...
  const id = `${sourceId}-new-${randomUUID().slice(0, 6)}`;
  await sql`INSERT INTO hr_surveys (id,title,description,status,anonymous) VALUES (${id},${`${source.title} · новый запуск`},${source.description},'draft',true)`;
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    if (req.method === "GET") {
      ...
      const analytics = selected ? await getAnalytics(selected) : null;
      const diagnostics = await getSurveyDiagnostics();
      const campaigns = selected ? await listSurveyCampaigns(selected) : [];
      res.status(200).json({ surveys, activeSurvey, selectedSurveyId: selected, analytics, diagnostics, campaigns });
      return;
    }
    if (req.method !== "POST") { res.status(405).json({ error: "Method not allowed" }); return; }
    const body = (req.body ?? {}) as any;
    if (body.action === "survey.ensureDefault") { const id = await ensureDefaultSurvey(); res.status(200).json({ ok: true, id, message: "Базовый HR-опрос создан или обновлён." }); return; }
    ...
    if (body.action === "survey.send" && typeof body.id === "string") {
      if (body.confirm !== "SEND_SURVEY") { res.status(400).json({ error: "Нужно подтвердить отправку" }); return; }
      const survey = (await listSurveys()).find((item) => item.id === body.id);
      if (!survey) { res.status(404).json({ error: "Опрос не найден" }); return; }
      const result = await sendSurveyInvite(survey.id, survey.title);
