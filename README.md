...
## Скриншоты товара без заказа

Режим `unordered_goods` обрабатывает присланные в MAX скриншоты поступления из 1С. Бот находит размеченную красную колонку, определяет строки с усиленной красной заливкой, распознаёт весь скриншот локальным русским OCR и отдельно читает каждую проблемную строку. Контрагент и реквизиты документа извлекаются из верхней части изображения, то есть им не требуется красная заливка.

Если операторы отправляют скриншоты в уже настроенный чат `price_changes`, менять его режим не требуется: текстовые сообщения продолжат формировать отчёты об изменении закупочных цен, а изображения будут автоматически направлены в OCR-контур.

```json
{
  "CHAT_ID_ЧАТА_ОПЕРАТОРОВ": {
    "name": "Товар без заказа",
    "mode": "unordered_goods",
    "enabled": true,
    "sendTo": "chat",
    "targetChatId": "CHAT_ID_КАТЕГОРИЙНЫХ_МЕНЕДЖЕРОВ"
  }
}
```

Для накопления статистики нужен `DATABASE_URL`. События и позиции сохраняются в `unordered_goods_events` и `unordered_goods_items`, повторная доставка одного MAX webhook не создаёт дубль. Защищённая статистика за последние 30 дней доступна после входа в админку через `GET /api/admin/unordered-goods`.

OCR работает локально через `tesseract.js` и русскую языковую модель, внешние API и ключи не требуются. На текущих шаблонах 1920×1080 алгоритм обнаружил соответственно 3 из 9, 5 из 11 и 4 из 25 видимых проблемных строк. При изменении масштаба или интерфейса 1С следует проверить алгоритм на новых примерах.

Если задан `OPENROUTER_API_KEY`, основное распознавание выполняет vision-модель OpenRouter. По умолчанию используется `qwen/qwen3.7-flash`; её можно заменить через `OPENROUTER_VISION_MODEL`. При ошибке OpenRouter бот автоматически возвращается к локальному Tesseract.
