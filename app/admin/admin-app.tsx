...
function Login({ onSuccess }: { onSuccess: () => void }) {
  ...
  return <main className="grid min-h-[100dvh] place-items-center px-4 py-8"><section className="grid w-full max-w-5xl overflow-hidden rounded-[2rem] border border-zinc-200 bg-white shadow-[0_32px_90px_-48px_rgba(24,24,27,0.35)] md:grid-cols-[1.1fr_0.9fr]"><div className="hidden min-h-[560px] flex-col justify-between bg-zinc-950 p-10 text-white md:flex"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-300">Суперцены</p><h1 className="mt-5 max-w-sm text-4xl font-semibold leading-[1.05] tracking-[-0.04em]">Единый центр управления ботом</h1><p className="mt-5 max-w-sm text-sm leading-6 text-zinc-400">Инструкции для сотрудников, обратная связь и HR-опросы — в одном рабочем пространстве.</p></div><div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl bg-white/10"><div className="bg-white/[0.04] p-4"><p className="text-xs text-zinc-500">Контент</p><p className="mt-1 text-sm font-medium">База знаний</p></div><div className="bg-white/[0.04] p-4"><p className="text-xs text-zinc-500">Аналитика</p><p className="mt-1 text-sm font-medium">HR-опросы</p></div></div></div><div className="flex flex-col justify-center p-7 md:p-10"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-800 md:hidden">Суперцены</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.03em]">Вход в админку</h2><p className="mt-2 text-sm leading-6 text-zinc-600">Используйте пароль администратора.</p><form className="mt-8 space-y-5" onSubmit={submit}><label className="block"><span className="mb-2 block text-sm font-medium">Пароль</span><input className="field" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" autoFocus required /></label>{error && <p className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700" role="alert">{error}</p>}<button className="button-primary w-full" disabled={pending}>{pending ? "Проверяем…" : "Войти"}</button></form></div></section></main>;
}
...
function Stats({ stats }: { stats: KnowledgeStats }) {
  ...
  return <dl className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-200 md:grid-cols-5">{items.map(([label, value]) => <div className="bg-white px-4 py-4 md:px-5" key={label}><dt className="text-xs leading-5 text-zinc-500">{label}</dt><dd className="mt-1 text-2xl font-semibold tracking-[-0.03em] tabular-nums">{value}</dd></div>)}</dl>;
}
